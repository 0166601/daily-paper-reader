...
tags: ["query:q3"]
score: 8.0
evidence: 网约车平台中通知订单请求的最优司机子集选择
...
method: 建立了通知集合选择问题的数学模型，并针对两种不同的冲突解决协议设计了多项式时间近似方案（PTAS）和基于子模性优化的近似算法。
result: 证明了该问题在两种协议下均为强NP难，但成功为FA协议提供了4倍近似算法，并为BA协议开发了超越(1-1/e)限制的优化算法。
conclusion: 非独占通知机制能有效提升网约车匹配质量，所提算法在理论复杂度和实际应用性能之间取得了良好的平衡。
---

## 摘要
网约车平台越来越多地依赖非排他性通知——即同时向多名司机广播单个请求——以缓解由司机接单不确定性导致的效率低下。本文是与 Lyft 合作的两部分研究中的第一部分，我们正式建模了单个决策周期内的“通知集合选择问题”，即平台为每个进入的行程请求确定要通知的最佳司机子集。我们在两种冲突解决协议下分析了这一组合优化问题：“优先接受 (FA)”协议通过将行程分配给首位响应者来优先考虑速度，而“最优接受 (BA)”协议通过选择价值最高的接单司机来优先考虑匹配质量。我们证明，在这两种机制下实现福利最大化都是强 NP 难的，从而排除了完全多项式时间近似方案 (FPTAS) 的可能性。尽管如此，我们推导出了若干积极的算法结果。对于 FA，我们针对单乘客情形提出了多项式时间近似方案 (PTAS)，并针对通用匹配场景提出了常数因子近似（因子为 4）。我们强调，FA 估值函数可以被视为一种具有独立理论价值的新型离散选择模型。对于 BA，我们证明了其目标函数是单调且次模的，允许标准的 (1 - 1/e) 近似。此外，利用我们为该问题设计的多项式时间需求预言机，我们证明了超越 (1 - 1/e) 限制的可能性。最后，在接单概率同质的特殊情况下，我们证明了 BA 问题可以通过线性规划建模在多项式时间内精确求解。我们通过在合成数据以及利用 Lyft 真实共享出行数据校准的实例上进行数值实验，验证了算法的经验性能。
