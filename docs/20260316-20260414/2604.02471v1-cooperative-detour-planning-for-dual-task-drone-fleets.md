...
title_zh: 双任务无人机集群的协同绕行规划
authors: "Pengbo Zhu, Meng Xu, Andreas A. Malikopoulos, Nikolas Geroliminis"
date: 2026-04-02
pdf: "https://arxiv.org/pdf/2604.02471v1"
tags: ["query:q4"]
score: 8.0
evidence: 无人机机队执行带有绕路规划的配送任务
tldr: 本文针对城市无人机机群同时执行快递配送与交通监控的双重任务，提出了一种去中心化的协作绕路规划框架。该框架通过动态局部聚类策略，使无人机在通信范围内共享交通状态信息并进行局部联合路径优化，在满足电池寿命和最大绕路限制的前提下，最大化城市交通信息的覆盖收益。研究在巴塞罗那城市网络中验证了该方法在降低计算开销的同时，能达到接近全局最优的监控效果。
source: arxiv
...
motivation: 旨在利用商业配送无人机机群作为移动传感器网络，解决实时城市交通监控中覆盖范围与资源利用的协同优化难题。
method: 提出一种基于动态局部聚类的去中心化框架，通过混合整数线性规划（MILP）在无人机相遇时进行局部联合路径重规划。
result: 仿真结果显示，该方法比传统最短路径策略能更有效地利用绕路预算获取交通信息，且计算效率显著优于中心化方案。
conclusion: 这种“相遇即合并”的去中心化策略为大规模无人机机群在复杂城市环境下的多任务协同提供了一种高效且接近最优的解决方案。
---

## 摘要
随着城市空中交通规模的扩大，商业无人机集群作为移动传感器网络用于实时城市交通监测，提供了一个极具吸引力但尚未得到充分探索的机会。本文提出了一个去中心化框架，使无人机集群能够同时执行配送任务并观测路网交通状况。我们利用与路段相关的动态信息值对城市环境进行建模，这些信息值随时间累积交通状况的不确定性，并在无人机访问时重置。该问题被建模为一个混合整数线性规划问题，无人机在遵守每次配送的最大绕行限制和每架无人机电池预算的前提下，最大化交通信息奖励。与在大规模集群中计算负担沉重的中心化方法不同，我们的方法侧重于动态局部聚类。当无人机进入通信范围时，它们会交换对交通状态的置信度，并从孤立的路径规划转变为局部联合优化模式，通过解决耦合约束分别为每架无人机获取重新规划的路径。基于西班牙巴塞罗那真实城市路网的仿真结果表明，与忽略交通监测任务的最短路径策略相比，本文提出的方法能更好地利用电池和绕行预算来探索城市区域并获取充足的交通信息；此外，得益于其去中心化的特性，这种“相遇与合并”策略在路网覆盖方面实现了接近全局最优的效果，且与中心化基准方法相比显著降低了计算开销。
