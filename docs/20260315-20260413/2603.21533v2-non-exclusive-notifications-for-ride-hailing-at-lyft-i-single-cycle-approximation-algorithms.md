...
score: 9.0
evidence: Lyft网约车调度与司机通知优化
tldr: 本文针对网约车平台中司机接单不确定性导致的效率低下问题，研究了“非独占通知”策略，即同时向多名司机发送订单请求。通过建立单周期通知集合选择模型，分析了“先到先得”和“择优录取”两种协议下的组合优化难题。研究证明了问题的计算复杂性，并提出了多种具有理论保证的近似算法，通过Lyft真实数据验证了该策略在提升匹配效率和平台福利方面的显著作用。
source: arxiv
...
motivation: 旨在通过向多个司机同时发送订单通知，解决因司机拒绝订单而导致的匹配延迟和平台效率损失问题。
method: 将通知集合选择建模为组合优化问题，针对先到先得（FA）和择优录取（BA）两种协议分别设计了近似算法和需求预言机。
result: 证明了该问题为强NP难，并为FA协议提供了4倍近似算法，为BA协议提供了优于(1-1/e)的近似保证及特定情况下的精确解。
conclusion: 非独占通知机制能有效优化网约车匹配质量，所提算法在理论复杂度和实际应用性能之间取得了良好平衡。
---
