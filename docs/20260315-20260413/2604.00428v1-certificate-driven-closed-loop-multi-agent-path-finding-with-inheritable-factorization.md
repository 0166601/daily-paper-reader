...
score: 8.0
evidence: 自动化仓库与物流中的多智能体路径规划
tldr: 针对多智能体路径规划（MAPF）中闭环算法在密集场景下因视野短浅导致性能下降的问题，本文提出了一种证书驱动的冲突搜索算法（CDCBS）。该方法通过引入证书轨迹作为冲突回退方案，并利用预算机制过滤更新以保证完备性，同时结合可继承的分解技术降低计算复杂度，显著提升了密集环境下的规划质量和效率。
source: arxiv
...
motivation: 现有的闭环MAPF算法在有限计算预算下容易因视野短浅而无法保证全局最优性，且难以利用问题的组合结构。
method: 提出证书轨迹和车队预算机制来过滤更新并提供回退方案，同时引入基于预算的可继承分解技术实现跨时步的全局任务拆解。
result: 在基准测试中，CDCBS在密集场景下比ACCBS表现出更稳定的解质量，且分解技术有效减小了智能体分组规模。
conclusion: 该框架通过证书驱动和结构分解，成功解决了闭环MAPF中的短视问题，为大规模多智能体协调提供了更高效、可靠的方案。
---

## 摘要
自动化仓库和物流中的多智能体协作通常被建模为多智能体路径规划（MAPF）问题。闭环 MAPF 算法通过仅规划下一步行动并进行在线重规划来提高可扩展性，但这种有限时域视角可能具有局限性，且难以维持全局保证和利用组合结构。这一问题在随时闭环冲突搜索（ACCBS）中尤为明显，该算法在动态扩展的有限时域上应用冲突搜索（CBS），但在有限的计算预算下，在密集实例中可能会以较短的活动前缀终止。我们引入了凭证轨迹及其相关的车队预算，作为过滤闭环更新的通用机制。凭证提供了一个无冲突的备选方案和剩余成本的单调上界；仅接受改进凭证的更新可确保完备性。相同的预算信息诱导了一种受预算限制的分解，从而实现了跨时间步的全局、可继承分解。在 ACCBS 上实例化该框架得到了凭证驱动的冲突搜索（CDCBS）。在基准地图上的实验表明，CDCBS 比 ACCBS 获得了更一致的解质量，特别是在密集场景下，同时所提出的分解方法减小了有效分组规模。
